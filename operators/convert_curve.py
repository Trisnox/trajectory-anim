...
def convert_curve_to_annotation(context: bpy.types.Context):
...
class CurveToAnnotation(Operator):
    """Convert curve to annotation stroke. Keyframe will be added correspond to current frame on timeline"""
    bl_idname = "trajectanim.curve_to_annotation"
    bl_label = "Convert curve into annotation"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        result = convert_curve_to_annotation(context)
        if not result:
            self.report({'INFO'}, 'Nothing to convert')
        else:
            self.report({'INFO'}, 'Successfully convert curve into annotation')
